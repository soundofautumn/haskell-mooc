...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node parent left right) = 1 + treeSize left + treeSize right
...
treeMax Empty = 0
treeMax (Node parent left right) = max parent $ max (treeMax left) (treeMax right)
...
allValues condition (Node parent left right) =
  condition parent
...
mapTree f (Node parent left right) = Node (f parent) (mapTree f left) (mapTree f right)
...
cull _ Empty = Empty
cull val (Node parent left right) =
  if parent == val
    then Empty
    else Node parent (cull val left) (cull val right)
...
isOrdered (Node parent left right) =
  allValues (< parent) left
    && isOrdered left
    && allValues (> parent) right
    && isOrdered right
...
walk [] (Node parent _ _) = Just parent
walk (s : ss) (Node parent left right) =
  case s of
    StepL -> walk ss left
    StepR -> walk ss right
...
set (p : ps) val (Node parent left right) =
  case p of
    StepL -> Node parent (set ps val left) right
    StepR -> Node parent left (set ps val right)
...
search val (Node parent left right) =
  if val == parent
    then Just []
