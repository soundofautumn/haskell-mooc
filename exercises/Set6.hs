...
instance Eq Name where
  (==) :: Name -> Name -> Bool
  (Name x) == (Name y) = length x == length y && map toLower x == map toLower y
...
simplify (RationalNumber ca cb) = RationalNumber a b
  where
    c = gcd ca cb
    a = div ca c
    b = div cb c
...
instance Num RationalNumber where
  (RationalNumber 0 _) + (RationalNumber 0 _) = RationalNumber 0 1
  p + (RationalNumber 0 _) = simplify p
  (RationalNumber 0 _) + q = simplify q
