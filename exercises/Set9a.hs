...
countValid (x : xs) = isValid x + countValid xs
  where
    isValid x = if x !! 2 == x !! 4 || x !! 3 == x !! 5 then 1 else 0
...
repeated [] = Nothing
repeated (x : []) = Nothing
repeated (x : x' : xs) = if x == x' then Just x else repeated (x' : xs)
...
sumSuccess (x : xs) = combine x $ sumSuccess xs
  where
    combine (Right x) (Right y) = Right (x + y)
    combine (Right x) _ = Right x
    combine _ (Right y) = Right y
    combine _ _ = Left "no data"
...
data Lock = LockOpened String | LockClosed String
  deriving (Show)
...
open x (LockClosed code) = if x == code then LockOpened code else LockClosed code
open _ (LockOpened code) = LockOpened code

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (LockOpened code) = LockClosed code
lock x = x

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode x (LockOpened code) = LockOpened x
changeCode _ x = x
...
instance Eq Text where
  (Text a) == (Text b) = noWhite a == noWhite b
    where
      noWhite [] = []
      noWhite (x : xs) = if isSpace x then noWhite xs else x : noWhite xs
...
permute p n = map snd (sortBy (\(p1, n1) (p2, n2) -> compare p1 p2) $ zip p n)
