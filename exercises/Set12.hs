...
inBoth foldx foldy = foldr (\x rest -> if elem x foldy then x : rest else rest) [] foldx
...
instance Foldable TwoList where
  ...
  foldr f init (TwoNode x y rest) = f x $ f y $ foldr f init rest
...
instance Functor Tree where
  fmap _ Leaf = Leaf
  fmap f (Node p l r) = Node (f p) (fmap f l) (fmap f r)

sumTree :: (Monoid m) => Tree m -> m
sumTree Leaf = mempty
sumTree (Node p l r) = sumTree l <> p <> sumTree r
