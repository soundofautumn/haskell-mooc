...
longest [xs] = xs
longest (xs : xs' : xss) =
  case compare (length xs) (length xs') of
    LT -> longest (xs' : xss)
    EQ -> case compare (head xs) (head xs') of
      LT -> longest (xs : xss)
      EQ -> longest (xs : xss)
      GT -> longest (xs' : xss)
    GT -> longest (xs : xss)
...
incrementKey _ [] = []
incrementKey k ((k', v') : kvs) = if k == k' then (k', v' + 1) : incrementKey k kvs else (k', v') : incrementKey k kvs
...
transfer from to amount bank =
  if Map.member from bank && Map.member to bank && amount > 0 && (bank Map.! from) >= amount
...
maxIndex arr = fst $ foldl1 maxByValue (assocs arr)
  where
    maxByValue acc@(_, maxVal) x@(_, val)
      | val > maxVal = x
      | otherwise = acc
