...
instance Transform Flip where
  apply FlipX (Picture picture) = Picture (\(Coord x y) -> picture (Coord (-x) y))
  apply FlipY (Picture picture) = Picture (\(Coord x y) -> picture (Coord x (-y)))
  apply FlipXY (Picture picture) = Picture (\(Coord x y) -> picture (Coord y x))
...
instance Transform Blur where
  apply Blur (Picture picture) = Picture avg
    where
      avg (Coord x y) = Color r g b
        where
          neighbors = [Coord (x - 1) y, Coord x (y - 1), Coord x y, Coord (x + 1) y, Coord x (y + 1)]
          colors = map picture neighbors
